...
// ==UserScript==
// @name         [Working] Bypass Key All Executors Make By AfxcaxiRoblox
// @homepageURL  https://discord.gg/7smFqq9KBP
...
// @license      MIT
// @supportURL   https://discord.gg/7smFqq9KBP
...
async function fluxus() {
    ...
    try {
        ...
        if (fluxusLink && fluxusLinkRegex.test(fluxusLink)) {
            ...
            document.getElementById('noUserscript').innerHTML = '<h1>DO NOT directly enter your fluxus link! Always come to this page directly.</h1><h2>Please Wait For Our Bypass. . .<br>(to prevent this delay, come to this page directly instead of going to fluxus key link)</h2>';
            await sleep(16000);
        }
...
async function hohohub() {
    ...
    try {
        ...
        let session = document.cookie.split('; ').find(cookie => cookie.includes('session')).split('=')[1];
        ...
        let nonce = document.getElementsByTagName('button')[1].onclick.toString()
        nonce = nonce.split(',')[1].split(')')[0].trim();
        nonce = parseInt(nonce);

        let start = await (await fetch('/api/start', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                site: 2,
                cucmep: btoa(nonce + 10 + 1000 + 9 + 12 + 6),
                session: session
            }),
        })).json();
...
function handleError(error) {
    ...
    GM_notification({
        text: errorText,
        title: "ERROR",
        url: 'https://discord.gg/7smFqq9KBP',
        silent: true,
    });
    GM.openInTab('https://discord.gg/7smFqq9KBP');
}
...
async function start() {
    ...
    if (window.location.hostname != 'keyrblx.com') {
        injectAds();
        alert('Please Wait, Soon...\n(Please Join My Discord Is: https://discord.gg/7smFqq9KBP Thank You For Using My Script.)');
    }

    GM_notification({
        text: 'Please Wait, Soon...\n(Please Join My Discord Is: https://discord.gg/7smFqq9KBP Thank You For Using My Script.)',
        title: "INFO",
        url: 'https://discord.gg/7smFqq9KBP',
        silent: true,
        timeout: 5000
